...
class AMOSDataset(Dataset):
    def __init__(self, name, root, mode, size=None, id_path=None, nsample=None):
        ...
        if mode == 'train_l' or mode == 'train_u':
            ...
            split = id_path.replace('splits/amos/', '').replace('/labeled.txt', '')
            print(split)
            with open(f'splits/%s/val.txt' % name, 'r') as f:
                self.ids = f.read().splitlines()
    ...
    def __getitem__(self, item):
        ...
        # print(f'before image max: {img.max()}, image min: {img.mean()}')
        img = img.astype(np.float32)
        # print(f'middle image max: {img.max()}, image min: {img.mean()}')
        # p5 = np.percentile(img.flatten(), 0.5)
        # p95 = np.percentile(img.flatten(), 99.5)
        # # print(p5, p95)
        # img = img.clip(min=p5, max=p95)
        # # normalize
        img = img.clip(min=-75, max=275)
        if img.max() - img.min() != 0:
            img = (img - img.min()) / (img.max() - img.min())
        # print(f'after image max: {img.max()}, image min: {img.mean()}')
