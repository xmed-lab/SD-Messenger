...
def main():
    ...
    for epoch in range(epoch + 1, cfg['epochs']):
        ...
        for i, ((img_x, mask_x), img_u_s) in enumerate(loader):
            ...
            # print(f'class weight: {weight_diff}')
            ...
            # print(f'cut_mix_area: {torch.sum(cut_mix_area)}, loss: {loss_consis}')
            if epoch > 5:
            ...
            if (i % (len(trainloader_u) // 8) == 0) and (rank == 0):
                logger.info(
                    'Iters: {:}, Total loss: {:.3f}, Loss x: {:.3f}, Loss w_fp: {:.3f}, Loss consistency: {:.6f}'
